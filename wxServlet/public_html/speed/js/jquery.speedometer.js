...
*/(function ($) {
    $.fn.speedometer = function (options) {
        ...
        var def = {
            ...
            percentage : $.trim($this.html()) || 0, scale : 100, limit : true, minimum : 0, maximum : 100, suffix : 'K', animate : true, thisCss :  {
                position : 'relative', /* Very important to align needle with gague. */
                width : '245px', height : '210px', padding : '0px', border : '0px', fontFamily : 'Arial', fontWeight : '900', backgroundImage : "url('./background.jpg')"
            },
            digitalCss :  {
                backgroundColor : 'black', borderColor : '#555555 #999999 #999999 #555555', borderStyle : 'solid', borderWidth : '2px', color : 'white', fontSize : '14px', height : '20px', left : '88px', padding : '1px', position : 'absolute', textAlign : 'center', top : '65px', width : '70px', zIndex : '10', lineHeight : '20px', overflow : 'hidden'
            }
        }
        ...
        if (def.limit && (def.percentage > def.maximum || def.percentage < def.minimum)) {
            ...
            $this.jqcanvas(function (canvas, width, height) {
                var ctx = canvas.getContext("2d"), lingrad, thisWidth;
                ctx.lineWidth = 2;
